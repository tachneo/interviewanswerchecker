...
# Disable SSL verification globally (for development purposes)
...
file_menu.add_command(label="Save Evaluation", command=lambda: save_evaluation())
file_menu.add_command(label="Load Evaluation", command=lambda: load_evaluation())
...
# Input Tab Layout
title_label = ttk.Label(input_tab, text="Interview Evaluation Tool", font=("Helvetica", 22, "bold"))
title_label.grid(row=0, column=0, columnspan=2, pady=(0, 20))

candidate_name_label = ttk.Label(input_tab, text="Candidate Name:", font=("Helvetica", 14, "bold"))
candidate_name_label.grid(row=1, column=0, sticky="w", pady=(5, 2))
candidate_name_entry = ttk.Entry(input_tab, width=70, font=("Helvetica", 12))
candidate_name_entry.grid(row=1, column=1, pady=(5, 2), sticky="w")

question_label = ttk.Label(input_tab, text="Interview Question:", font=("Helvetica", 14, "bold"))
question_label.grid(row=2, column=0, sticky="w", pady=(5, 2))
question_text = scrolledtext.ScrolledText(input_tab, height=4, width=70, font=("Helvetica", 12),
                                          wrap="word", bd=2, relief="groove")
question_text.grid(row=3, column=0, columnspan=2, pady=(0, 15))

cand_answer_label = ttk.Label(input_tab, text="Candidate's Answer:", font=("Helvetica", 14, "bold"))
cand_answer_label.grid(row=4, column=0, sticky="w", pady=(5, 2))
cand_answer_text = scrolledtext.ScrolledText(input_tab, height=6, width=70, font=("Helvetica", 12),
                                             wrap="word", bd=2, relief="groove")
cand_answer_text.grid(row=5, column=0, columnspan=2, pady=(0, 15))

buttons_frame = ttk.Frame(input_tab)
...
evaluation_output = scrolledtext.ScrolledText(evaluation_tab, height=20, width=100, font=("Helvetica", 12),
                                              wrap="word", bd=2, relief="groove")
evaluation_output.pack(pady=10)
evaluation_output.config(state=tk.DISABLED)

# Reporting Tab Layout
scorecard_title = ttk.Label(reporting_tab, text="Score Card", font=("Helvetica", 20, "bold"))
scorecard_title.pack(pady=(0, 10))

score_label = ttk.Label(reporting_tab, text="Score: N/A", font=("Helvetica", 16))
score_label.pack(pady=(0, 10))

chart_container = ttk.Frame(reporting_tab)
chart_container.pack(pady=(0, 15), fill=tk.BOTH, expand=True)

details_frame = ttk.LabelFrame(reporting_tab, text="Evaluation Details", padding="10")
...
options_frame = ttk.LabelFrame(reporting_tab, text="Options", padding="10")
...
# Global variable for chart canvas
chart_canvas = None

# Functions
def evaluate_candidate_answer():
    """Start evaluation in a separate thread to keep GUI responsive."""
    thread = threading.Thread(target=perform_evaluation)
    thread.start()

def perform_evaluation():
    """Evaluate candidate answer using Gemini API."""
...
def display_formatted_evaluation(evaluation):
    """Display evaluation with color-coded formatting."""
    evaluation_output.config(state=tk.NORMAL)
    evaluation_output.delete("1.0", tk.END)
...
def update_chart(evaluation_text):
    """Update the score chart based on evaluation."""
    global chart_canvas
    score_match = re.search(r"Score:\s*(.+)", evaluation_text, re.IGNORECASE)
    score = int(score_match.group(1).strip()) if score_match and score_match.group(1).strip().isdigit() else 0
    score_label.config(text=f"Score: {score_match.group(1).strip()}" if score_match else "Score: N/A")
...
def update_details(evaluation_text):
    """Update strengths, weaknesses, and recommendation in reporting tab."""
...
def clear_fields():
    """Clear all fields and reset displays."""
...
def save_evaluation():
    ...
    data = {
        ...
        'answer': cand_answer_text.get('1.0', 'end-1c'),
        'evaluation': evaluation_output.get('1.0', 'end-1c')
    }
    file_path = filedialog.asksaveasfilename(defaultextension='.json', filetypes=[('JSON files', '*.json')])
    if file_path:
        with open(file_path, 'w') as f:
            json.dump(data, f)
        messagebox.showinfo('Save Success', 'Evaluation saved successfully!')
        status_bar.config(text="Evaluation saved.")
...
def load_evaluation():
    ...
    if file_path:
        ...
        candidate_name_entry.insert(0, data['candidate_name'])
        question_text.delete('1.0', 'end')
        question_text.insert('1.0', data['question'])
        cand_answer_text.delete('1.0', 'end')
        cand_answer_text.insert('1.0', data['answer'])
        evaluation_output.config(state='normal')
        evaluation_output.delete('1.0', 'end')
        evaluation_output.insert('1.0', data['evaluation'])
        evaluation_output.config(state='disabled')
        update_chart(data['evaluation'])
        if show_details_var.get():
            update_details(data['evaluation'])
        messagebox.showinfo('Load Success', 'Evaluation loaded successfully!')
        status_bar.config(text="Evaluation loaded.")
...
def export_evaluation():
    ...
    if file_path:
        try:
            ...
            status_bar.config(text="Evaluation exported.")
        except Exception as e:
            messagebox.showerror("Export Error", f"Error exporting evaluation: {e}")

def generate_certificate():
    """Generate a certificate in a new window."""
...
# Configure evaluation output tags
